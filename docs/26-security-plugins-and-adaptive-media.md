...
| P1 | A Hyphae append without an expected cursor accepted a substituted next position. | Absence now means signed/derivable genesis; cursor position must equal the final signed server sequence, timestamps are semantic RFC3339, and legacy ACK reads are incrementally bounded. |
