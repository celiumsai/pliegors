...
| G1 Native runtime and dynamic rendering | In progress | [Runtime](evidence/g1-native-runtime-foundation.md), [native-socket](evidence/g1-native-socket-foundation.md), [complete-render](evidence/g1-complete-render-foundation.md), [ordered-render](evidence/g1-ordered-render-foundation.md), and [dynamic-reference](evidence/g1-dynamic-reference-foundation.md) evidence cover the unreleased route graph, lifecycle, raw TCP HTTP/1.1 path, two bounded SSR modes, and one launchable native application. Gate closure still requires async boundaries, middleware/errors, HTTP/2, fixed-load and bounded-memory evidence, OTel, security evidence, and no unresolved P0. |
