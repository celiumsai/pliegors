...
| G1 Native runtime and dynamic rendering | In progress | [Runtime](evidence/g1-native-runtime-foundation.md), [native-socket](evidence/g1-native-socket-foundation.md), [complete-render](evidence/g1-complete-render-foundation.md), and [ordered-render](evidence/g1-ordered-render-foundation.md) evidence cover the unreleased route graph, lifecycle, raw TCP HTTP/1.1 path, and two bounded SSR modes. Gate closure still requires async boundaries, middleware/errors, HTTP/2, the dynamic reference application under fixed load, bounded memory, OTel, security evidence, and no unresolved P0. |
