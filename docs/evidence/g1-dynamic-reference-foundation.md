...
- complete HTML, ordered HTML, CSS, unknown-route diagnostics, and bind policy
  passed;
- Clippy with warnings denied passed; and
- doc tests passed.

The launched binary also passed a loopback HTTP smoke against `/`, `/stream`,
`/health`, and `/assets/site.css`. The observed response bodies were 790, 815,
51, and 1,183 bytes respectively, and `SIGINT` completed a clean process exit.
...
HTTP/2, TLS, middleware, authored error boundaries, asynchronous render
boundaries, OpenTelemetry, multipart/decompression policy, or deployment from
a sealed release candidate. It does not close G1 or change any capability from
`not-released`.
