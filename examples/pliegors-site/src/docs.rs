...
pub const TOPICS: &[DocTopic] = &[
    ...
    DocTopic {
        slug: "portable-deployment",
        group_en: "Delivery",
        group_es: "Entrega",
        title_en: "Portable deployment",
        title_es: "Despliegue portable",
        summary_en: "Validate one sealed PBOC, admit native or Cloudflare hosts, and prove rolling and rollback compatibility before upload.",
        summary_es: "Valida un PBOC sellado, admite hosts nativos o Cloudflare y demuestra compatibilidad rolling y rollback antes del upload.",
    },
    DocTopic {
        slug: "fullstack-beta",
        group_en: "Runtime",
...
pub fn article(locale: Locale, slug: &str) -> Result<View, String> {
    ...
    let content = match slug {
        ...
        "portable-deployment" => portable_deployment(locale),
        "fullstack-beta" => fullstack_beta(locale),
    ...
    vec![
        ...
        doc_section(locale, "transport", "Bound the network before application code", "Limita la red antes del código de aplicación", vec![
            ...
            definition_list(locale, &[
                ...
                ("Body formats", "PLG-RUN-110 rejects conflicting framing with 400. The published G1 generic path rejects encoded or multipart bodies; current main adds G2 action-only decoded and part budgets.", "PLG-RUN-110 rechaza framing conflictivo con 400. La ruta genérica del G1 publicado rechaza bodies encoded o multipart; main agrega presupuestos G2 de decoding y partes solo para actions."),
        ...
        doc_section(locale, "layouts", "Bind the document to its route-owned layout", "Vincula el documento con el layout controlado por su ruta", vec![
            paragraph(locale, "A sealed match exposes the complete group/layout scope chain separately from the layout-only chain. LayoutDocument requires one LayoutLayer for every matched layout and composes them root to leaf. Typed before, after, and wrap operations transform one private child frame, so application code cannot omit or duplicate it. No HTML string is parsed or replaced.", "Un match sellado expone por separado la cadena completa de scopes group/layout y la cadena exclusiva de layouts. LayoutDocument exige un LayoutLayer por cada layout del match y los compone de raíz a hoja. Las operaciones tipadas before, after y wrap transforman un único frame hijo privado, por lo que el código de aplicación no puede omitirlo ni duplicarlo. Ningún string HTML se parsea o reemplaza."),
            ...
            note(locale, "Data boundary", "Complete, ordered, and async-boundary responses accept route-owned layouts. Current main binds the G2 source-beta loader and resource contracts to the request lifecycle; the published G1 component does not include them.", "Límite de datos", "Las respuestas complete, ordered y async-boundary aceptan layouts controlados por rutas. Main vincula los contratos source-beta G2 de loaders y recursos al lifecycle del request; el componente G1 publicado no los incluye."),
        ]),
        doc_section(locale, "runtime-otel", "Attach operator-owned OpenTelemetry", "Conecta OpenTelemetry controlado por el operador", vec![
            ...
            code_block(locale, "rust", "use pliego_runtime::{\n    HttpScheme, OpenTelemetryConfig, RemoteTracePolicy,\n};\n\nlet telemetry = OpenTelemetryConfig::new(HttpScheme::Https)\n    .known_method(\"PROPFIND\")?\n    .remote_trace_policy(RemoteTracePolicy::AcceptW3c);\n\nlet runtime = NativeRuntimeBuilder::new(graph, \"production-a\")?\n    .open_telemetry(telemetry)\n    .build()?;"),
            definition_list(locale, &[
                ("Default", "No instrumentation and no accepted remote parent", "Sin instrumentación ni parent remoto aceptado"),
                ("Scheme", "Explicit trusted Http or Https operator value", "Valor Http o Https explícito y confiable del operador"),
                ("Metrics", "Request duration, active requests, and response body size", "Duración de request, requests activos y tamaño del body de respuesta"),
                ("Cardinality", "Known method set, sealed route template, finite framework error codes", "Set de métodos conocidos, template de ruta sellado y códigos de framework finitos"),
...
fn portable_deployment(locale: Locale) -> View {
    vec![
        ...
        doc_section(locale, "verify", "Verify before upload", "Verifica antes del upload", vec![
            paragraph(locale, "Validation checks canonical structure, every artifact path, byte length, SHA-256, the exact file set, route and function references, cache policy, feature closure, provenance, and the boundary that keeps secret values out of output.", "La validación comprueba estructura canónica, cada ruta de artefacto, longitud, SHA-256, el set exacto de archivos, referencias de rutas y funciones, política de cache, cierre de features, provenance y el límite que mantiene valores secretos fuera de la salida."),
            code_block(locale, "shell", "pliego pboc validate target/app/pliego.pboc.json --root target/app\npliego pboc admit target/app/pliego.pboc.json --root target/app --host native\npliego pboc admit target/app/pliego.pboc.json --root target/app --host cloudflare"),
            note(locale, "Fail closed", "Missing, additional, modified, linked, non-portable, oversized, or unsupported required artifacts and features are rejected before a provider upload begins.", "Falla cerrado", "Artefactos o features requeridos ausentes, adicionales, modificados, enlazados, no portables, sobredimensionados o incompatibles se rechazan antes de iniciar un upload al proveedor."),
        ]),
        doc_section(locale, "hosts", "Run the same build on two hosts", "Ejecuta el mismo build en dos hosts", vec![
            definition_list(locale, &[
                ("Native Linux / OCI", "Static musl executable, verified route and runtime identities, nonroot reference image, read-only filesystem, and explicit operator ingress", "Ejecutable musl estático, identidades de ruta y runtime verificadas, imagen de referencia nonroot, filesystem de solo lectura e ingress explícito del operador"),
                ("Cloudflare Workers", "Rust workers-rs adapter, exact function registry, secret handles mapped after admission, native Worker streaming, and an Assets binding", "Adapter Rust workers-rs, registro exacto de funciones, handles de secretos mapeados después de la admisión, streaming nativo de Worker y binding de Assets"),
                ("Shared semantics", "One router, PBOC SHA, release identity, HTTP status/body/header corpus, cache policy, and bounded diagnostics", "Un router, SHA PBOC, identidad de release, corpus de estados/body/headers HTTP, política de cache y diagnósticos limitados"),
            ]),
            paragraph(locale, "Provider account IDs, credentials, domains, traffic policy, TLS, billing, observability destinations, and persistent infrastructure remain operator configuration outside the portable bundle.", "IDs de cuenta del proveedor, credenciales, dominios, política de tráfico, TLS, billing, destinos de observabilidad e infraestructura persistente siguen como configuración del operador fuera del bundle portable."),
        ]),
        doc_section(locale, "compatibility", "Prove rolling and rollback direction", "Demuestra la dirección rolling y rollback", vec![
            code_block(locale, "shell", "pliego pboc compatibility active/pliego.pboc.json candidate/pliego.pboc.json --direction rolling\npliego pboc compatibility candidate/pliego.pboc.json active/pliego.pboc.json --direction rollback"),
            paragraph(locale, "Rolling coexistence requires the same application, compatibility epoch, and state schema, a higher sequence, and an exact previous-release link. Rollback additionally requires a lower sequence and an active release that declared rollbackSafe.", "La coexistencia rolling exige la misma aplicación, epoch de compatibilidad y schema de estado, una secuencia mayor y un vínculo exacto al release anterior. Rollback exige además una secuencia menor y que el release activo haya declarado rollbackSafe."),
            note(locale, "State remains real", "A declaration cannot reverse an irreversible database or business mutation. Stateful applications must attach migration and provider-store evidence to their release decision.", "El estado sigue siendo real", "Una declaración no puede revertir una mutación irreversible de base de datos o negocio. Las aplicaciones con estado deben adjuntar evidencia de migración y del store del proveedor a su decisión de release."),
        ]),
        doc_section(locale, "conformance", "Reproduce provider conformance", "Reproduce la conformidad de proveedores", vec![
            code_block(locale, "shell", "npm ci\nnpm ci --prefix examples/provider-tck\nnpm run check:provider-tck"),
            paragraph(locale, "The Linux gate builds two releases, records four host admissions, checks negative capability and state cases, scans all bundle bytes for a secret sentinel, replays version skew and rollback, and compares seven HTTP cases across native, OCI, and Cloudflare.", "El gate Linux compila dos releases, registra cuatro admisiones de host, comprueba casos negativos de capabilities y estado, escanea todos los bytes del bundle con un sentinel secreto, reproduce version skew y rollback y compara siete casos HTTP entre native, OCI y Cloudflare."),
            link_list(locale, &[
                ("https://github.com/celiumsai/pliegors/blob/main/docs/50-pboc-portable-deployment.md", "Read the complete deployment guide", "Leer la guía completa de despliegue"),
                ("https://github.com/celiumsai/pliegors/blob/main/docs/evidence/g3-pboc-provider-conformance.md", "Inspect G3 acceptance evidence", "Inspeccionar evidencia de aceptación G3"),
                ("https://github.com/celiumsai/pliegors/blob/main/security/asvs-v5.0.0-g3.json", "Inspect the G3 security ownership map", "Inspeccionar el mapa de ownership de seguridad G3"),
            ]),
        ]),
        doc_section(locale, "limits", "Know what is not portable yet", "Conoce lo que aún no es portable", vec![
            paragraph(locale, "The maintained preview hosts are native Linux/OCI and Cloudflare Workers. Portable databases, queues, schedules, object storage, durable objects, provider billing, automatic state migration, and asynchronous-boundary parity are not part of PBOC v1alpha1.", "Los hosts mantenidos del preview son native Linux/OCI y Cloudflare Workers. Bases de datos, queues, schedules, object storage, durable objects, billing del proveedor, migración automática de estado y paridad de boundaries asíncronos no forman parte de PBOC v1alpha1."),
            link_list(locale, &[
                ("/docs/native-runtime", "Return to native runtime ownership", "Volver a la propiedad del runtime nativo"),
                ("/docs/build-and-deploy", "Separate static and application deployment", "Separar despliegue estático y de aplicaciones"),
            ]),
        ]),
    ].into_view()
}
...
fn fullstack_beta(locale: Locale) -> View {
    vec![
        ...
        doc_section(locale, "model", "Seal data authority before serving", "Sella la autoridad de datos antes de servir", vec![
            ...
            definition_list(locale, &[
                ("DataContext", "Admitted values, deadline, cancellation, cleanup, grants, and redacted receipts", "Valores admitidos, deadline, cancelación, cleanup, grants y recibos redactados"),
                ("LoaderPolicy", "Typed input/output schemas, immutable bounded output, resources, and request-local deduplication", "Schemas tipados de entrada/salida, output inmutable limitado, recursos y deduplicación local al request"),
                ("ActionPolicy", "Strict input, auth hooks, commit state, idempotency, invalidation intent, and navigation", "Entrada estricta, hooks de auth, estado de commit, idempotencia, intención de invalidación y navegación"),
                ("Runtime contract", "Canonical route, resource, loader, action, cache, and invalidation projection with one SHA-256", "Proyección canónica de rutas, recursos, loaders, actions, cache e invalidación con un SHA-256"),
            ]),
        ]),
        doc_section(locale, "progressive", "Keep the mutation useful without JavaScript", "Mantén la mutación útil sin JavaScript", vec![
            paragraph(locale, "The reference login and account mutation are ordinary HTML forms. The same action policy admits form, JSON, gzip, or multipart only when explicitly configured; it validates Origin, session-bound CSRF, authentication, application authorization, unknown fields, encoded and decoded bytes, and output before application code receives typed input.", "El login y la mutación de cuenta de referencia son formularios HTML ordinarios. La misma política de action admite form, JSON, gzip o multipart solo cuando se configura explícitamente; valida Origin, CSRF vinculado a sesión, autenticación, autorización de aplicación, campos desconocidos, bytes codificados y decodificados y output antes de que el código de aplicación reciba entrada tipada."),
            definition_list(locale, &[
                ("Idempotency", "Binds key, action revision, principal partition, admitted input, and deployment epoch", "Vincula key, revisión del action, partición del principal, entrada admitida y epoch de deployment"),
                ("Commit", "Distinguishes no commit, committing, committed, outcome unknown, and compensation required", "Distingue sin commit, committing, committed, outcome unknown y compensación requerida"),
                ("Uploads", "Bounds fields, parts, filenames, files, total temporary storage, decoding, and cleanup", "Limita fields, partes, filenames, archivos, storage temporal total, decoding y cleanup"),
                ("Failures", "Returns typed accessible 422 errors, 403 CSRF rejection, or 409 idempotency conflict", "Devuelve errores 422 tipados y accesibles, rechazo CSRF 403 o conflicto idempotente 409"),
            ]),
        ]),
        doc_section(locale, "sessions-cache", "Separate identity from cache authority", "Separa identidad de autoridad de cache", vec![
            paragraph(locale, "Opaque server-side sessions rotate at login, reject schema skew and expiry, and revoke through the configured store. Session claims identify a principal; application authorization still runs next to protected resources. Cache policies independently declare public, private-request, or private-session domains, exact Vary, structured partitions, freshness, bounds, compatibility epoch, and fill behavior.", "Las sesiones opacas server-side rotan en login, rechazan incompatibilidad de schema y expiración, y se revocan mediante el store configurado. Los claims identifican un principal; la autorización de la aplicación sigue ejecutándose junto a los recursos protegidos. Las políticas de cache declaran de forma independiente dominios public, private-request o private-session, Vary exacto, particiones estructuradas, freshness, límites, epoch de compatibilidad y comportamiento de fill."),
            note(locale, "Read-your-writes", "An action may emit only the cache policy and exact tag target declared in its sealed contract. Read-your-writes navigation waits for the configured acknowledgement barrier; a missing acknowledgement fails explicitly.", "Read-your-writes", "Un action solo puede emitir la política de cache y el target exacto de tags declarado en su contrato sellado. La navegación read-your-writes espera la barrera de acknowledgements configurada; un acknowledgement ausente falla explícitamente."),
        ]),
        doc_section(locale, "diagnostics", "Inspect contracts and receipts", "Inspecciona contratos y recibos", vec![
            code_block(locale, "shell", "cargo run -p fullstack-pliego -- contract .pliego/runtime-contract.json\ncargo run -p pliego-cli -- inspect action rename-account --contract .pliego/runtime-contract.json\ncargo run -p pliego-cli -- why request runtime-receipt.json\ncargo run -p pliego-cli -- why cache cache-receipt.json"),
            paragraph(locale, "Diagnostic inputs are bounded, versioned, and contract-checked. Explanations show stable policy identity, coarse outcome, commit and acknowledgement state, and digests while excluding bodies, claims, identities, raw cache keys, values, and secrets.", "Las entradas diagnósticas son limitadas, versionadas y verificadas contra contrato. Las explicaciones muestran identidad estable de política, outcome aproximado, estado de commit y acknowledgements y digests, mientras excluyen bodies, claims, identidades, keys de cache crudas, valores y secretos."),
            link_list(locale, &[
                ("https://github.com/celiumsai/pliegors/blob/main/docs/evidence/g2-fullstack-beta.md", "Read the G2 acceptance evidence", "Leer la evidencia de aceptación G2"),
                ("https://github.com/celiumsai/pliegors/blob/main/docs/rfc/RFC-010-data-actions-cache.md", "Read the G2 umbrella RFC", "Leer el RFC principal de G2"),
                ("https://github.com/celiumsai/pliegors/blob/main/security/asvs-v5.0.0-g2.json", "Inspect the ASVS ownership map", "Inspeccionar el mapa de ownership ASVS"),
            ]),
        ]),
    ].into_view()
}
...
fn build_and_deploy(locale: Locale) -> View {
    vec![
        ...
        doc_section(locale, "deploy", "Deploy the static output", "Despliega la salida estática", vec![
            ...
            link_list(locale, &[("/docs/portable-deployment", "Deploy a server application with PBOC", "Desplegar una aplicación de servidor con PBOC")]),
        ]),
    ].into_view()
}
    ...
    vec![
        ...
        doc_section(locale, "symbols", "Core public entry points", "Entradas públicas principales", vec![
            definition_list(locale, &[
                ...
                ("pliego_pboc::{PbocManifest, HostProfile, verify_bundle}", "Validate exact deployment output and negotiate one target before upload", "Valida salida exacta de despliegue y negocia un target antes del upload"),
                ("pliego_cloudflare::Application", "Bind an admitted PBOC to an exact Rust Worker function registry", "Vincula un PBOC admitido a un registro exacto de funciones de un Worker Rust"),
...
fn outline(slug: &str) -> Vec<(&'static str, &'static str, &'static str)> {
    match slug {
        ...
        "portable-deployment" => vec![
            ("contract", "PBOC preview", "Preview PBOC"),
            ("verify", "Verify and admit", "Verificar y admitir"),
            ("hosts", "Native and Cloudflare", "Native y Cloudflare"),
            (
                "compatibility",
                "Rolling and rollback",
                "Rolling y rollback",
            ),
            ("conformance", "Provider TCK", "TCK de proveedores"),
            ("limits", "Current limits", "Límites actuales"),
        ],
        "fullstack-beta" => vec![
            ...
            ("diagnostics", "Diagnostics", "Diagnósticos"),
        ],
        "opensdk" => vec![
