...
The source tree also contains the first bounded `complete` server-rendering
mode over `pliego-dom`. It emits a typed HTML document or fragment, validates
metadata and response status, and binds the render mode into the runtime
receipt before response commitment.
...
This foundation is intentionally incomplete. It does not yet expose ordered or
boundary streaming, middleware phases, OpenTelemetry, multipart/decompression
