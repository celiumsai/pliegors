...
use pliego_fold::{
    CANONICAL_JSON_CODEC_ID, CanonicalJsonCodec, CodecError, EventResolveError, EventResolver,
    MAX_PROJECTION_SNAPSHOT_BYTES, Projection, ProjectionError, ProjectionSnapshot, ReactiveLog,
    Reducer, ReducerError, ReducerIdentity, SnapshotError, StateCodec,
};
use pliego_log::{Event, EventCatalogBuilder, EventSchema, Hash, Log};
...
impl EventResolver<Delta> for Resolver {
    fn schema_set_digest(&self) -> Hash {
        self.digest
    }

    fn resolve(&self, event: &Event) -> Result<Delta, EventResolveError> {
        if self.panic_at == Some(event.seq()) {
            panic!("injected resolver panic");
        }
        if event.kind() != Delta::KIND || event.schema_version() != Delta::VERSION {
            return Err(EventResolveError::new("unsupported test schema"));
        }
        serde_json::from_slice(event.payload().as_bytes())
...
fn r3_a09_snapshot_binds_history_schema_reducer_codec_and_state() {
    ...
    assert_eq!(snapshot.schema_set_digest(), &[9; 32]);
...
fn r3_a18_schema_err_and_panic_never_publish_candidate_state() {
    let raw = log_of(&events([(5, "ok"), (7, "ok")]));
...
fn r3_a20_live_genesis_and_snapshot_tail_are_equal_for_deterministic_cases() {
    for seed in 0_u64..16 {
        ...
        let full = log_of(&generated);
        let live = projection(full.clone());
        let expected = live.get();
        assert_eq!(expected, projection(full.clone()).get(), "seed={seed}");
        for cut in 0..=generated.len() {
            let prefix = log_of(&generated[..cut]);
            let snapshot_source = projection(prefix.clone());
            let snapshot = snapshot_source.snapshot().unwrap();
            let mut restored_log = prefix;
            for event in &generated[cut..] {
                restored_log.append_typed(event).unwrap();
            }
            let restored = Projection::restore(
                ReactiveLog::from_log(restored_log),
                snapshot,
                Resolver::new(9),
                reducer(3),
                CanonicalJsonCodec::default(),
...
fn r3_a22_sealed_catalog_upcasts_version_mix_before_reduction() {
    ...
    builder
        .register_current::<Delta, _>(|event| event)
        .unwrap()
